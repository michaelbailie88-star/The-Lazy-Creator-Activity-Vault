...
exports.handler = async (event) => {
  ...
  const store = getStore("vault-reviews");
  ...
  if (event.httpMethod === "POST") {
    try {
      ...
      const now = Date.now();
      if (_lastSubmit[ip] && now - _lastSubmit[ip] < RATE_WINDOW_MS) {
        return { statusCode: 429, headers, body: JSON.stringify({ error: "Please wait a moment before submitting another review." }) };
      }
      ...
      _lastSubmit[ip] = now;
      return { statusCode: 200, headers, body: JSON.stringify({ review, reviews: list }) };
    } catch (e) {
      return { statusCode: 500, headers, body: JSON.stringify({ error: "Could not save review. Please try again." }) };
    }
  }
